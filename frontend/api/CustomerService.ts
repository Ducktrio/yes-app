...
export default class CustomerService {
    ...
    static async getCustomers(query?: GetCustomerQuery): Promise<Customer[]> {
        return await api.get<Customer[]>("/Customers", { params: query })
            .then((res) => {
                return res.data;
            })
            .catch((err) => {
    ...
    static async updateCustomer(id: string, customer: Partial<CreateCustomerPayload>) {
        return await api.put<Customer>(`/Customers?id=${id}`, customer)
            .then((res) => {
                return res.data;
            })
            .catch((err) => {
    ...
    static async deleteCustomer(id: string) {
        return await api.delete(`/Customers?id=${id}`)
            .then((res) => {
                return res.data;
            })
            .catch((err) => {
