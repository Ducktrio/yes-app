...
export default class UserService {
    ...
    static async getUsers(query?: GetUserQuery): Promise<User[]> {
        return await api.get<User[]>("/Users", { params: query })
            .then((res) => {
                return res.data;
            })
            .catch((err) => {
    ...
    static async getUserById(id: string) {
        await api.get<User>(`/Users?id=${id}`)
            .then((res) => {
                return res.data;
            })
            .catch((err) => {
    ...
    static async getUserByRole(roleId:string) {
        await api.get<User[]>(`/Users?role_id=${roleId}`)
            .then((res) => {
                return res.data;
            })
            .catch((err) => {
    ...
    static async getUserByUsername(username: string) {
        await api.get<User[]>(`/Users?username=${username}`)
            .then((res) => {
                return res.data;
            })
            .catch((err) => {
        ...
        await api.post<User>("/Users", user)
            ...
            .catch((err) => {
                if(err.response.status === 400) { // Validation error
                    throw new ValidationError(err.response.data as ValidationErrorResponse[]);
                }
                if (err.response) {
                    throw new Error(err.response.data.title);
                }
    ...
    static async updateUser(id: string, user: Partial<CreateUserPayload>) {
        ...
        await api.put<User>(`/Users/${id}`, user)
            .then((res) => {
                return res.data;
            })
            .catch((err) => {
